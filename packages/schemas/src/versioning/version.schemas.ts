import type { ContractVersion, Version, VersionParts } from '../../../contracts/src/versioning/types';
...
function parseParts(value: string): VersionParts {
  ...
  if (!match) throw new TypeError('Expected stable semantic version in MAJOR.MINOR.PATCH form');
...
function makeVersionSchema<T extends string>(): VersionStringSchema<T> {
  const is = (value: unknown): value is T => typeof value === 'string' && STABLE_SEMVER_CORE_PATTERN.test(value);
  return Object.freeze({
    is,
    parse(value: unknown): T {
      if (!is(value)) throw new TypeError('Expected stable semantic version in MAJOR.MINOR.PATCH form');
      return value;
    },
    serialize(value: T): string {
      if (!is(value)) throw new TypeError('Cannot serialize invalid semantic version');
      return value;
    },
    parts(value: T): VersionParts {
...
export const SupportedContractVersionSchema = Object.freeze({
  ...
  is(value: unknown): value is ContractVersion {
    ...
    return CONTRACT_VERSION_REGISTRY.supportedRead.some((supported) => String(supported) === value);
  },
  parse(value: unknown): ContractVersion {
    const parsed = ContractVersionSchema.parse(value);
    if (!CONTRACT_VERSION_REGISTRY.supportedRead.some((supported) => String(supported) === parsed)) {
