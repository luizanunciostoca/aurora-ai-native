...
function outside(input: TenantBoundaryCheck, reason: TenantBoundaryReason): TenantBoundaryDecision {
  const actorBindings = input.bindings.filter((binding) => binding.identityId === input.context.actor.identityId);
  return {
    ...
    evidence: {
      ...
      matchedBindingCount: actorBindings.filter((binding) => binding.tenantId === input.context.tenantId).length,
...
export function checkTenantBoundary(input: TenantBoundaryCheck): TenantBoundaryDecision {
  if (!input.knownTenantIds.includes(input.context.tenantId)) return outside(input, 'TENANT_UNKNOWN');

  const { actor, subject } = input.context;
  if (subject.kind === 'IDENTITY' && subject.identityId !== actor.identityId) return outside(input, 'SUBJECT_MISMATCH');
  if (subject.kind === 'EXTERNAL_IDENTITY') {
    if (actor.externalIdentity === undefined) return outside(input, 'EXTERNAL_IDENTITY_MISMATCH');
    if (
  ...
  const targetBindings = actorBindings.filter((binding) => binding.tenantId === input.context.tenantId);

  if (targetBindings.length === 0) {
    return outside(input, actorBindings.length > 0 ? 'CROSS_TENANT_IDENTITY' : 'IDENTITY_NOT_BOUND');
  }
  ...
  if (actor.kind === 'SYSTEM' && binding.bindingKind !== 'SYSTEM') return outside(input, 'BINDING_KIND_MISMATCH');
  if (actor.kind !== 'SYSTEM' && binding.bindingKind === 'SYSTEM') return outside(input, 'BINDING_KIND_MISMATCH');

  const actorExternal = actor.externalIdentity;
  if (binding.bindingKind === 'EXTERNAL' || actorExternal !== undefined) {
    if (!sameExternalIdentity(binding.externalIdentity, actorExternal)) return outside(input, 'EXTERNAL_IDENTITY_MISMATCH');
  }
