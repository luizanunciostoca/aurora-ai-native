...
function target(overrides: Partial<ProviderExecutionTargetReference> = {}): ProviderExecutionTargetReference {
  return {
    schemaVersion: VERSION,
    kind: 'PROVIDER',
    provider: 'META',
...
test('W08-C stops at page budget and scopes continuation cursor to the exact request', async () => {
  ...
  const first = await executeProviderRead(
    request({ limits: { maxPages: 1, maxItems: 10 } }),
    { credentials: credentialBackend(), adapter },
  );
