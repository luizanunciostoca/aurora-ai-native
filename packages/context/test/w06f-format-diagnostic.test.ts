...
test('W06-F reports exact Prettier differences for the current candidate', async () => {
  for (const file of [
    'packages/context/src/semantic-cache/index.ts',
    'packages/context/src/semantic-cache/types.ts',
    ...
    console.log('[w06f:prettier-diff]', JSON.stringify({ file, ...diff }));
