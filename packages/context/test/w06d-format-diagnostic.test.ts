...
test('W06-D reports exact Prettier differences for the current candidate', async () => {
  for (const file of [
    'packages/context/src/snapshots/index.ts',
    'packages/context/test/w06d-context-snapshot.test.ts',
  ]) {
    const before = await readFile(file, 'utf8');
    const config = (await resolveConfig(file)) ?? {};
    const after = await format(before, { ...config, filepath: file });
    const diff = differingWindow(before, after);
    console.log('[w06d:prettier-diff]', JSON.stringify({ file, ...diff }));
