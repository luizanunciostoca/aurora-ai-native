...
function diffHunks(before: string, after: string) {
  ...
  while (i < a.length || j < b.length) {
    if (i < a.length && j < b.length && a[i] === b[j]) {
      ops.push({ kind: 'same', line: a[i]! }); i += 1; j += 1; continue;
    }
    if (j < b.length && (i === a.length || (dp[i]![j + 1] ?? 0) >= (dp[i + 1]![j] ?? 0))) {
      ops.push({ kind: 'add', line: b[j]! }); j += 1; continue;
    }
    ops.push({ kind: 'delete', line: a[i]! }); i += 1;
  }
  const hunks: typeof ops[] = [];
  let current: typeof ops = [];
  for (const op of ops) {
    if (op.kind === 'same') {
      if (current.length > 0) { hunks.push(current); current = []; }
