...
test('W06-C reports exact Prettier differences for the current candidate', async () => {
  for (const file of [
    'packages/context/src/minimal-context/compile.ts',
    'packages/context/test/w06c-minimal-context.test.ts',
  ]) {
    const before = await readFile(file, 'utf8');
    const config = (await resolveConfig(file)) ?? {};
    const after = await format(before, { ...config, filepath: file });
    const diff = differingWindow(before, after);
    console.log('[w06c:prettier-diff]', JSON.stringify({ file, ...diff }));
