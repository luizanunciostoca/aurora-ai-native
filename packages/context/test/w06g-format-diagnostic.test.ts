...
test('W06-G reports exact Prettier hunks for candidate files', async () => {
  for (const file of [
    'packages/context/src/speculation/index.ts',
    'packages/context/src/speculation/types.ts',
    'packages/context/test/w06g-safe-speculation.test.ts',
  ]) {
    const before = await readFile(file, 'utf8');
    const config = (await resolveConfig(file)) ?? {};
    const after = await format(before, { ...config, filepath: file });
    const changes = hunks(before, after);
    console.log('[w06g:prettier-hunks]', JSON.stringify({ file, changes }));
