...
function complexity(input: TaskClassificationInput, resolvedModality: TaskModality): TaskComplexity {
  const estimatedSteps = boundedCount(
    input.complexityDrivers.estimatedSteps,
    'TaskClassificationInput.complexityDrivers.estimatedSteps',
  );
  const dependencyCount = boundedCount(
    input.complexityDrivers.dependencyCount,
    'TaskClassificationInput.complexityDrivers.dependencyCount',
  );
  const externalInteractionCount = boundedCount(
    input.complexityDrivers.externalInteractionCount,
    'TaskClassificationInput.complexityDrivers.externalInteractionCount',
  );

  const hasStructuralDriver =
    estimatedSteps !== undefined ||
    dependencyCount !== undefined ||
    externalInteractionCount !== undefined ||
    input.complexityDrivers.requiresSpecializedTool !== undefined;

  if (!hasStructuralDriver) return 'UNKNOWN';

  const score =
    driverWeight(estimatedSteps, [1, 8]) +
    driverWeight(dependencyCount, [1, 4]) +
    driverWeight(externalInteractionCount, [1, 3]) +
    (input.complexityDrivers.requiresSpecializedTool ? 1 : 0) +
    (resolvedModality === 'MULTIMODAL' ? 1 : 0) +
    (input.operation === 'EXECUTE' ? 1 : 0);
...
function riskSignals(input: TaskClassificationInput): readonly TaskRiskSignal[] {
  ...
  if (input.ambiguity === 'HIGH' || input.ambiguity === 'UNKNOWN') {
    signals.add('AMBIGUOUS_REQUIREMENTS');
  }
  if (
...
function reasonCodes(
  ...
  resolvedModality: TaskModality,
  resolvedComplexity: TaskComplexity,
): readonly ClassificationReasonCode[] {
  const reasons: ClassificationReasonCode[] = [];
  reasons.push(
    input.operation === 'UNKNOWN' ? 'TASK_CLASS_UNKNOWN_OPERATION' : 'TASK_CLASS_FROM_OPERATION',
  );
  reasons.push(
    resolvedModality === 'UNKNOWN'
      ? 'MODALITY_MISSING'
      : resolvedModality === 'MULTIMODAL'
        ? 'MODALITY_MULTIPLE'
        : 'MODALITY_SINGLE',
  );
  reasons.push(
    resolvedComplexity === 'UNKNOWN'
      ? 'COMPLEXITY_INSUFFICIENT_DRIVERS'
      : 'COMPLEXITY_FROM_BOUNDED_DRIVERS',
  );
  reasons.push(
    input.sideEffectProfile === 'UNKNOWN'
  ...
  if (input.sideEffectProfile === 'IRREVERSIBLE') {
    reasons.push('RISK_IRREVERSIBILITY_DERIVED');
  }
  if (input.ambiguity === 'HIGH' || input.ambiguity === 'UNKNOWN') {
    reasons.push('RISK_AMBIGUITY_DERIVED');
  }
  if (
    input.evidenceCompleteness === 'INSUFFICIENT' ||
    input.evidenceCompleteness === 'UNKNOWN'
  ) {
    reasons.push('RISK_INSUFFICIENT_EVIDENCE_DERIVED');
  }
...
/**
 * Pure deterministic classifier over normalized, bounded task facts.
 * It never evaluates current policy/authority and has no side-effect capability.
 */
