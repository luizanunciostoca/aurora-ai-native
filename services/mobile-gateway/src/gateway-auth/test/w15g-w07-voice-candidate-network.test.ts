...
test(
  'forwards only bounded candidate plus server-derived context and sanitizes successful evaluation',
  () => {
...
test(
  'rejects client-supplied identity policy trust outcome and retry fields before W07 intake',
  () => {
    let calls = 0;
    const boundary = new VoiceCandidateNetworkBoundary({
      evaluate: () => {
...
test(
  'rejects malformed, authority-bearing, or unavailable W07 responses without exposing details',
  () => {
