...
const SCHEMA_VERSION = '1.0.0' as ContractVersion;
...
const ATTEMPTED_AT = '2026-09-05T19:30:00.000Z' as Rfc3339Timestamp;
const LATE_ATTEMPTED_AT = '2026-09-05T19:30:02.000Z' as Rfc3339Timestamp;
...
test('authenticated device receipt becomes attempt receipt and UNVERIFIED execution evidence only', () => {
  ...
  if (result.evidence === undefined) throw new Error('expected execution receipt evidence');
  ...
  assert.equal(result.receipt.acknowledgedAt, '2026-09-05T19:30:01.500Z');
  assert.equal(result.receipt.returnedAt, '2026-09-05T19:30:01.500Z');
  assert.equal(result.receipt.executionOutcome, undefined);
  assert.equal(result.receipt.executionTarget.kind, 'DEVICE');

  assert.equal(evidence.evidenceType, 'EXECUTION_RECEIPT');
  assert.equal(evidence.verification.state, 'UNVERIFIED');
  assert.equal(evidence.source.sourceType, 'EXECUTOR');
  assert.equal(evidence.readback, undefined);
  assert.equal(evidence.capturedAt, '2026-09-05T19:30:01.500Z');
  assert.equal(evidence.metadata?.deviceReportedState, 'COMPLETED');
  assert.equal(evidence.metadata?.requiresW07Reconciliation, false);
  assert.equal(evidence.metadata?.deviceCapturedAtMs, CAPTURED_AT_MS);
  assert.equal('authenticationReference' in (evidence.metadata ?? {}), false);
  assert.equal('integrityDigest' in (evidence.metadata ?? {}), false);
});
...
test('trusted execution binding must match tenant correlation device command execution and timing', () => {
  ...
  source.factory = (lookup) =>
    material(lookup, { actionIntent: actionIntent('dvc_01ARZ3NDEKTSV4RRFFQ69G5FAW') });
  let result = adapter.observe(observation());
  assert.equal(result.ok, false);
  if (!result.ok) assert.equal(result.code, 'CONFLICT');

  source.factory = (lookup) =>
    material(lookup, { commandId: 'cmd_01ARZ3NDEKTSV4RRFFQ69G5FAW' as CommandId });
  result = adapter.observe(observation());
  assert.equal(result.ok, false);
  if (!result.ok) assert.equal(result.code, 'CONFLICT');

  source.factory = (lookup) => material(lookup, { attemptedAt: LATE_ATTEMPTED_AT });
  result = adapter.observe(observation());
  assert.equal(result.ok, false);
  if (!result.ok) assert.equal(result.code, 'CONFLICT');
});
