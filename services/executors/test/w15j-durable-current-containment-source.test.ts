...
function record(): DurableContainmentStateRecord {
  return {
    ...
    circuitKey: CIRCUIT,
    version: 2,
    updatedAt: '2026-09-06T00:29:59.000Z' as Rfc3339Timestamp,
    snapshot: {
      circuit: { state: 'CLOSED', consecutiveFailures: 0, halfOpenProbeInFlight: false },
      killSwitch: { state: 'INACTIVE', changedAt: '2026-09-06T00:29:59.000Z' as Rfc3339Timestamp },
...
test('resolves circuit key and current durable snapshot on every voice-state read', () => {
  ...
  assert.equal(
    source.resolve({ actionIntent: intent(), tenantId: TENANT, evaluatedAt: NOW })?.circuit.state,
    'CLOSED',
  );
  assert.equal(
    source.resolve({ actionIntent: intent(), tenantId: TENANT, evaluatedAt: NOW })?.killSwitch
      .state,
    'INACTIVE',
  );
...
test('tenant mismatch, missing key, missing state and owner outages all fail closed', () => {
  ...
  assert.equal(
    missingKey.resolve({ actionIntent: intent(), tenantId: TENANT, evaluatedAt: NOW }),
    null,
  );
  ...
  assert.equal(
    missingState.resolve({ actionIntent: intent(), tenantId: TENANT, evaluatedAt: NOW }),
    null,
  );
  ...
  assert.equal(
    throwing.resolve({ actionIntent: intent(), tenantId: TENANT, evaluatedAt: NOW }),
    null,
  );
});
