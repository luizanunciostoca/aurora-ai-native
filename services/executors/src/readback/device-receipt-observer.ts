...
function materialMatchesObservation(
  ...
  return (
    ...
    attemptedAtMs !== null &&
    attemptedAtMs <= observation.capturedAtMs &&
...
export class W07DeviceReceiptObservationAdapter {
  ...
  observe(input: unknown): DeviceReceiptObservationResult {
    ...
    const capturedAt = timestampFromMs(observation.capturedAtMs);
    if (capturedAt === null) return rejected('MALFORMED');
