...
if (!/^[0-9a-f]{40}$/i.test(expectedHead || ""))
  fail("valid expected HEAD SHA is required");
if (!/^[0-9a-f]{40}$/i.test(expectedMain || ""))
  fail("valid expected main SHA is required");
...
if (Buffer.byteLength(text, "utf8") > 2_000_000)
  fail("acceptance output exceeds size limit");
const lines = text
  .split(/\r?\n/)
  .map((line) => line.trim())
  .filter(Boolean);
const markers = lines.filter((line) =>
  line.startsWith("AURORA_ACCEPTANCE_RESULT="),
);
if (markers.length !== 1)
  fail("exactly one AURORA_ACCEPTANCE_RESULT marker is required");
...
if (result.repository !== expectedRepository)
  fail("acceptance output repository mismatch");
if (result.prNumber !== expectedPrNumber)
  fail("acceptance output PR number mismatch");
if (!["ACCEPT_RECOMMENDED", "REWORK_REQUIRED"].includes(result.decision)) {
  fail("invalid acceptance decision");
}
if (result.exactHead !== expectedHead)
  fail("acceptance output exact HEAD mismatch");
if (result.main !== expectedMain) fail("acceptance output main mismatch");
if (
  !result.riskGates ||
  typeof result.riskGates !== "object" ||
  Array.isArray(result.riskGates)
) {
  fail("riskGates object is required");
}
for (const gate of ["A", "B", "C", "D"]) {
  if (!["PASS", "FAIL"].includes(result.riskGates[gate]))
    fail(`invalid Risk Gate ${gate}`);
}
if (
  ...
  fail(
    "blockers must be at most 100 non-empty strings of at most 1000 characters",
  );
}
if (
...
if (result.decision === "ACCEPT_RECOMMENDED") {
  if (result.blockers.length !== 0)
    fail("ACCEPT_RECOMMENDED cannot contain blockers");
  for (const gate of ["A", "B", "C", "D"]) {
...
for (const name of requiredNames) {
  const matches = checkEvidence.requiredChecks.filter(
    (check) => check?.name === name,
  );
  if (matches.length !== 1)
    fail(`required check ${name} must appear exactly once`);
  const [check] = matches;
  if (
    ...
    !check.detailsUrl.startsWith(
      `https://github.com/${expectedRepository}/actions/runs/`,
    )
  ) {
    fail(
      `required check ${name} is not exact-head successful GitHub Actions evidence`,
    );
  }
}
...
const normalized = {
  ...
  requiredChecks: checkEvidence.requiredChecks
    .map((check) => ({
      ...
      completedAt:
        typeof check.completedAt === "string" ? check.completedAt : null,
    }))
