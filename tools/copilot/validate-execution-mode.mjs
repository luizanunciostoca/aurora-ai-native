...
if ((mode.schemaVersion || 1) < 5) throw new Error('Measured PRO+ activation requires schemaVersion >= 5');
...
if (!Number.isInteger(mode.maxLogicalLanes) || mode.maxLogicalLanes < mode.physicalBuildSlots || mode.maxLogicalLanes > 256)
  throw new Error('maxLogicalLanes must be between physicalBuildSlots and 256');
if (!Number.isInteger(mode.maxPrebuildArtifactLanes) || mode.maxPrebuildArtifactLanes < 0 || mode.maxPrebuildArtifactLanes > mode.maxLogicalLanes)
  throw new Error('maxPrebuildArtifactLanes must be between 0 and maxLogicalLanes');
if (typeof mode.prebuildWorkerEnabled !== 'boolean') throw new Error('prebuildWorkerEnabled must be boolean');

const discovery = mode.runtimeCapabilityDiscovery || {};
if (discovery.enabled !== true || discovery.requiredForProPlus !== true || !Array.isArray(discovery.signals) || discovery.signals.length < 5)
...
if (capacity.strategy !== 'MINIMUM_SAFE_DIMENSION' || capacity.subtractActiveSessionLeases !== true || capacity.respectWriterLeases !== true)
  throw new Error('Dynamic BUILD capacity controller policy is invalid');

const leases = mode.sessionLeaseRegistry || {};
if (leases.source !== 'LIVE_GITHUB_ISSUE_TASK_PROJECTION' || leases.prOpenConsumesBuildSession !== false || !Array.isArray(leases.lockDimensions) || !leases.lockDimensions.includes('allowedPaths') || !leases.lockDimensions.includes('sharedWriteSurfaces'))
  throw new Error('Session lease registry policy is invalid');

if (mode.developmentTelemetry?.enabled !== true || mode.developmentTelemetry?.canonicalAuthority !== false)
...
if (mode.mode === 'PRO_PLUS_ACTIONS_FABRIC') {
  if (mode.proPlusActionsFabricEnabled !== true || mode.cloudAgentEnabled !== false || mode.freeActionsCliEnabled !== true)
    throw new Error('PRO_PLUS_ACTIONS_FABRIC requires Actions fabric plus Free fallback and no cloud-agent claim');
  if (!Number.isInteger(mode.fallbackPhysicalBuildSlots) || mode.fallbackPhysicalBuildSlots < 1 || mode.fallbackPhysicalBuildSlots > 2)
    throw new Error('PRO+ Free fallback must stay between 1 and 2 slots');
  if (!Number.isInteger(discovery.minimumObservedConcurrentSessions) || discovery.minimumObservedConcurrentSessions < 3)
...
console.log(JSON.stringify({ schemaVersion: mode.schemaVersion, mode: mode.mode, activationState: mode.activationState || null, physicalBuildSlots: mode.physicalBuildSlots, fallbackPhysicalBuildSlots: mode.fallbackPhysicalBuildSlots || null, runtimeCapabilityDiscovery: discovery, capacityController: capacity, sessionLeaseRegistry: leases, developmentTelemetry: mode.developmentTelemetry, scheduler }, null, 2));
