...
export function discoverRuntimeCapabilities(mode, env = process.env, suppliedAttestation = null, nowMs = Date.now()) {
  ...
  const fleetOverride = positiveInteger(env.AURORA_FLEET_SUBAGENT_CAP || env.COPILOT_SUBAGENT_MAX_CONCURRENT);

  if (freeMode) {
    const slots = positiveInteger(mode?.physicalBuildSlots) || 1;
    return { schema: 'aurora.pro_plus.runtime_capability.v2', configuredMode, executionProfile: 'FREE', executionAvailable: Boolean(mode?.freeActionsCliEnabled), proPlusReady: false, attestationState: 'NOT_REQUIRED', isolatedSessionCapacity: slots, ciParallelCapacity: ciOverride || slots, creditSlotBudget: creditOverride || slots, fleetSubagentCap: fleetOverride || slots, authority: false };
  }
  ...
  if (actionsFabricMode) {
    ...
    const measuredSlots = verified ? Math.min(positiveInteger(mode?.physicalBuildSlots) || fallbackSlots, positiveInteger(attestation?.observedConcurrentSessions) || fallbackSlots) : fallbackSlots;
    return { schema: 'aurora.pro_plus.runtime_capability.v2', configuredMode, executionProfile: verified ? 'PRO_PLUS' : 'FREE_FALLBACK', executionAvailable: verified || Boolean(mode?.freeActionsCliEnabled), proPlusReady: verified, attestationState: verified ? 'VERIFIED' : attestation?.state || 'MISSING', isolatedSessionCapacity: measuredSlots, ciParallelCapacity: ciOverride || measuredSlots, creditSlotBudget: creditOverride || measuredSlots, fleetSubagentCap: fleetOverride || measuredSlots, authority: false };
  }
  ...
  const proPlusReady = Boolean(cloudMode && mode?.cloudAgentEnabled && cloudAgentAvailable && accountPlanObserved && isolatedSessionCapacity && ciOverride && creditOverride);
  return { schema: 'aurora.pro_plus.runtime_capability.v2', configuredMode, executionProfile: proPlusReady ? 'PRO_PLUS_CLOUD' : 'UNAVAILABLE', executionAvailable: proPlusReady, proPlusReady, attestationState: 'ENVIRONMENT_SIGNALS', isolatedSessionCapacity: isolatedSessionCapacity || 0, ciParallelCapacity: ciOverride || 0, creditSlotBudget: creditOverride || 0, fleetSubagentCap: fleetOverride || 0, authority: false };
}

export function calculateDynamicSafeBuildCapacity({ mode, runtime, readyCandidateCount, pathIndependentCandidateCount, activeLeaseCount = 0 }) {
  ...
  if (!runtime?.executionAvailable) return { schema: 'aurora.pro_plus.safe_build_capacity.v2', capacity: 0, reasons: ['RUNTIME_EXECUTION_UNAVAILABLE'], authority: false };
  ...
  const capacity = Math.max(0, Math.min(configuredCeiling, availableSessions, runtime.ciParallelCapacity, runtime.creditSlotBudget, ready, independent));
  const reasons = [];
  ...
  return { schema: 'aurora.pro_plus.safe_build_capacity.v2', capacity, executionProfile: runtime.executionProfile, dimensions: { configuredCeiling, availableSessions, ciParallelCapacity: runtime.ciParallelCapacity, creditSlotBudget: runtime.creditSlotBudget, readyCandidateCount: ready, pathIndependentCandidateCount: independent, activeLeaseCount: active }, reasons, authority: false };
}
