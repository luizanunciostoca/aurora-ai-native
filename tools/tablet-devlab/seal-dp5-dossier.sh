...
for path in "$TUPLE" "$DOSSIER" "$BINDER" "$PREFLIGHT" "$FINALIZE" "$APK_IDENTITY" "$FINAL_MANIFEST"; do
...
for required in "$(basename "$PRESEAL_MANIFEST")" "$(basename "$SEAL_STATUS")" "$(basename "$DOSSIER")"; do
...
printf 'dossier=%s\nfinal_manifest=%s\n' "$DOSSIER" "$FINAL_MANIFEST"
printf 'collector_manifest_sha256=%s\ndossier_sealed_sha256=%s\n' \
  "$preseal_manifest_sha" "$dossier_sealed_sha"
