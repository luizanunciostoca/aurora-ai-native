...
fail() {
  printf 'Aurora W15-J bootstrap reconnect failed: %s\n' "$*" >&2
  exit 2
}
...
REFRESH_FILE="$STATE_DIR/w15j-bootstrap-reconnect.json"
...
if [[ -e "$REFRESH_FILE" ]]; then
  secure_regular_file "$REFRESH_FILE" || fail "existing bootstrap reconnect output is insecure"
  rm -f -- "$REFRESH_FILE"
fi

kill -USR1 "$HOST_PID" || fail "could not signal the live host for reconnect bootstrap"

for (( attempt = 0; attempt < 50; attempt += 1 )); do
  [[ -e "$REFRESH_FILE" ]] && break
  sleep 0.1
done
secure_regular_file "$REFRESH_FILE" || fail "no secure reconnect output arrived; no established matching gateway session may exist or the provider principal may have expired"
...
jq -e \
  ...
  '
    ...
    .kind == "W15J_LOCAL_BOOTSTRAP_RECONNECT_READY" and
  ...
  ' "$REFRESH_FILE" >/dev/null || fail "reconnect output failed schema or authority validation"
...
(( REMAINING_MS > 30000 )) || fail "fresh reconnect bootstrap has less than 30 seconds remaining"
REMAINING_SECONDS="$(( REMAINING_MS / 1000 ))"

rm -f -- "$REFRESH_FILE"

cat <<EOF
W15J_BOOTSTRAP_RECONNECT=READY_NOT_ACCEPTED
...
Paste only bootstrap_reference into Aurora immediately. This reconnect bootstrap does not grant authority, retry authority, execution success or physical acceptance.
EOF
