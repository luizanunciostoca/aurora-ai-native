...
PACKAGING_HEAD_SHA="${AURORA_PACKAGING_HEAD_SHA:-fdeec9b16cccdb39ade1bf3e83755763072b9709}"
EXPECTED_PACKAGING_BRANCH="${AURORA_PACKAGING_BRANCH:-prototype/w15j-physical-apk-artifact-v017r12}"
RUN_ID="${AURORA_PACKAGING_RUN_ID:-35527423987}"
ARTIFACT_ID="${AURORA_ARTIFACT_ID:-10609684076}"
ARTIFACT_NAME="${AURORA_ARTIFACT_NAME:-aurora-w15j-v017r12-presign-apk-e42ea065-host-790049ae}"
EXPECTED_ZIP_SHA="${AURORA_ARTIFACT_ZIP_SHA256:-adada77c3b779a8de6f82da1eddc82a90037f82695509938ed4657c5a0c417d2}"
EXPECTED_PRESIGN_APK_SHA="${AURORA_PRESIGN_APK_SHA256:-04d84c63692a165b5e4ad480a5308779679b45cec41cf0d0fa69f83d67d84097}"
EXPECTED_FINAL_APK_SHA="${AURORA_APK_SHA256:-b72bae6efda9a1f85c2d0f08e108091c37f7648911bc25d242d05b4de4f62a0f}"
EXPECTED_CERT_SHA="${AURORA_PHYSICAL_SIGNER_CERT_SHA256:-e1745e3d3940fc6b03aef0b609d43aa8c436901965966087c2366108ffe263fb}"
EXPECTED_ANDROID_SHA="${AURORA_ANDROID_SHA:-e42ea0650a5056dd0d26e6ac69021691b5625332}"
EXPECTED_HOST_SHA="${AURORA_HOST_SHA:-790049ae133035db3ed1517b31d9110d7f20d9a4}"
...
embedded_branch="$(sed -n 's/^packaging_branch=//p' "$TMP/BUILD_IDENTITY.txt")"
...
[[ "$embedded_branch" == "$EXPECTED_PACKAGING_BRANCH" ]] || fail "embedded packaging branch drift: $embedded_branch"
