...
function redact(value) {
  return String(value)
    ...
    .replace(/\b(?:gbr|session|sess|credential|secret|token)_[A-Za-z0-9._-]{12,}\b/giu, "[REDACTED_SECRET_REF]")
    .replace(/(authorization\s*:\s*bearer\s+)[^\s]+/giu, "$1[REDACTED]")
    .replace(/((?:password|secret|credential|access[_-]?token)\s*[=:]\s*)[^\s]+/giu, "$1[REDACTED]");
}
...
function snapshot(attemptDir, phase, scenario, correlationId) {
  ...
  shell(
    "cpu.txt",
    "dumpsys cpuinfo 2>&1 | grep -i -E '" + PACKAGE_ID + "|TOTAL' | tail -n 120",
  );
...
function validatePreparedDossier(dossier) {
  ...
  if (dossier.device?.physicalDeviceVerified !== true) fail("verified physical device binding required");
...
function filesRecursive(directory, base = directory) {
  ...
  for (const entry of readdirSync(directory, { withFileTypes: true })) {
    ...
    else if (entry.isFile()) out.push({ path, relative: relative(base, path).replaceAll("\\", "/") });
  }
  return out;
}
...
function start(evidenceDir, id, requestedCorrelationId) {
  ...
  if (record.attempts.some((entry) => entry.status === "STARTED")) fail(id + " already has an active attempt");
  ...
  secureWrite(join(attemptDir, "operator-instructions.txt"), instructions(DP5_SCENARIO_BY_ID[id]) + "\n");
  manifest(attemptDir);
  saveCampaign(p.state, campaign);
  ...
  const notes = (title, values) =>
    ...
    (values.length
      ? values
          .map(
            (value) =>
              "- " + value.id + ": " + value.text + (value.reference ? " — " + value.reference : ""),
          )
          .join("\n")
      : "- None recorded.") +
    "\n";
