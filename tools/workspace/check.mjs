...
if (list) {
  ...
  for (const item of result.active) console.log(`  - ${item.dir} (${item.packageJson.name ?? 'unnamed'})`);
...
console.log(`[workspace] PASS: ${result.active.length} active package(s); ${result.excludedReferences.length} protected reference manifest(s) excluded.`);
