...
const reconnect = readFileSync(
  resolve(repoRoot, 'tools/tablet-devlab/refresh-reconnect.sh'),
  'utf8',
);
...
test('host launcher enables reconnect only through distinct protected DevLab state', () => {
  assert.match(runHost, /BOOTSTRAP_RECONNECT_FILE=.*w15j-bootstrap-reconnect\.json/);
  assert.match(runHost, /secure_regular_file.*BOOTSTRAP_RECONNECT_FILE/s);
  assert.match(
    runHost,
    /AURORA_W15J_BOOTSTRAP_RECONNECT_FILE=\/aurora-devlab\/state\/w15j-bootstrap-reconnect\.json/,
  );
  assert.doesNotMatch(runHost, /adb reverse/);
});

test('reconnect refresh binds SIGUSR1 output to the same proven host instance and remains non-authoritative', () => {
  assert.match(reconnect, /last-readiness-termux\.txt/);
  assert.match(reconnect, /host-ready-announcement\.txt/);
  assert.match(reconnect, /kill -0/);
  assert.match(reconnect, /127\.0\.0\.1:8080\/v1\/local-host\/instance/);
  assert.match(reconnect, /127\.0\.0\.1:8081\/v1\/local-host\/instance/);
  assert.match(reconnect, /kill -USR1/);
  assert.match(reconnect, /W15J_LOCAL_BOOTSTRAP_RECONNECT_READY/);
  assert.match(reconnect, /REMAINING_MS > 30000/);
  assert.match(reconnect, /authorizesExecution == false/);
  assert.match(reconnect, /provesExecutionSuccess == false/);
  assert.match(reconnect, /retryAuthorized == false/);
  assert.match(reconnect, /physical_acceptance=false/);
  assert.doesNotMatch(reconnect, /physical_acceptance=true/);
});
