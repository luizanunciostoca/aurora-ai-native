...
function runBinary(name, args) {
  ...
  if (!existsSync(binary)) {
    console.error(
      `QUALITY_TOOL_MISSING: ${name} was not found at ${path.relative(rootDir, binary)}. ` +
        'Install the root devDependencies selected by W00-A before running this gate.',
    );
    return 127;
  }
  ...
  for (const entry of readdirSync(directory, { withFileTypes: true })) {
    ...
    if (entry.isDirectory()) {
      if (!ignoredDirectoryNames.has(entry.name)) {
        collectTypeScriptProjects(absolutePath, projects);
      }
      continue;
    }
...
function typecheck() {
  ...
  if (uniqueProjects.length === 0) {
    console.log(
      'TYPECHECK_NO_PROJECTS_YET: no canonical tsconfig.json exists in runtime roots; ' +
        'tsconfig.base.json is ready for future TypeScript projects.',
    );
    return 0;
  }
...
function formatCheck() {
  return runBinary('prettier', ['--check', '.']);
}

function formatWrite() {
  return runBinary('prettier', ['--write', '.']);
}

function lint() {
  return runBinary('eslint', ['.', '--max-warnings=0']);
}
