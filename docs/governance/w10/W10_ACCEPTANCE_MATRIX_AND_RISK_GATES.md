...
- zero unresolved review threads or blocking findings;
- current-main and candidate-HEAD revalidation immediately before expected-head merge;
- expected-head merge guarded by the exact final candidate SHA;
- post-merge exact-main Quality + Test Build + Security before releasing descendants;
- only after post-merge verification may the owning task receive `aurora:accepted`.
...
| governed acceptance | Risk A-D COMMENT + zero blockers + expected-head merge + post-merge Q/TB/S | yes |
