...
- Free-plan subagent concurrency is capped at 2; the Aurora worker therefore claims at most two READY tasks per batch.
...
- Existing task graph, custom agents, ownership, exact-head CI, independent acceptance and no-self-merge rules remain unchanged.
