...
class AuroraWakeModelStore(context: Context) {
    ...
    private fun encode(model: AuroraWakeTemplateModel): String =
        buildString {
    ...
    private fun decode(payload: String): AuroraWakeTemplateModel {
        ...
        val templates =
            lines.drop(3).map { line ->
                WakeFeatureVector(line.split(',').map(String::toDouble))
            }
...
/**
 * Integrity protection only. Possession/use of this key is not Aurora action authority and cannot
 * mint an OwnerDecision, PolicyToken, execution result, verified outcome, or retry permission.
 */
private class AndroidKeystoreWakeModelIntegrity {
    ...
    fun verify(payload: String, encodedSignature: String): Boolean {
        val actual =
            runCatching { Base64.decode(encodedSignature, Base64.NO_WRAP) }.getOrNull()
                ?: return false
        val mac = Mac.getInstance(ALGORITHM)
        mac.init(key())
